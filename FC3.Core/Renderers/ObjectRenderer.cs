...
namespace FC3.Core.Renderers
{
	public class ObjectRenderer
	{
		public interface IListener
		{
			void ProcessObject(ObjectInventoryEntry entry, Image img);
		}
		...
		public static void RequestObjectImage(ObjectInventoryEntry entry)
		{
		}
		...
		private static void RenderObject(ObjectInventoryEntry entry)
		{
		...
		{
			string path = string.Format("{0}\\ingameeditor\\thumbnails\\png_src\\", Engine.Instance.GenericDataPath);
			...
			for (int i = 0; i < files.Length; i++)
			{
				...
				string text2 = string.Format("{0}\\ingameeditor\\thumbnails\\{1}\\", Engine.Instance.GenericDataPath, subDir);
